  ...
  return (
    <div>
      ...
      <div className="p-4 sm:p-8 max-w-2xl mx-auto">
        ...
        <form onSubmit={handleCreate} className="bg-white p-4 rounded shadow-sm mb-6 flex flex-col sm:flex-row gap-3">
          <input
          ...
          />
          <button className="w-full sm:w-auto bg-teal text-white rounded px-4 py-2 hover:opacity-90">Add</button>
        </form>
        {error && <p className="text-red-600 text-sm mb-4">{error}</p>}

        <div className="bg-white rounded shadow-sm divide-y">
          {subjects.map((s) => (
            <div key={s.id} className="p-4 flex flex-col sm:flex-row sm:justify-between sm:items-center gap-1">
